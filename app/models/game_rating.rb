class GameRating < ApplicationRecord
  ...
  after_save do
	self.game.update(ges_rating: self.game.game_ratings.average(:rating))
	...
	@rating.update(rating: self.team.game_ratings.where(rating_criterium_id: self.rating_criterium_id).average(:rating))
  end
end
